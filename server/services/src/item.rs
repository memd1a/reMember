use data::entities::{equip_item, inventory_slot, item_stack};
...
mod tests {
    ...
    async fn get_svc() -> anyhow::Result<(ItemService, AccountId, CharacterID)> {
        let db = gen_sqlite(data::SQL_OPT_MEMORY).await?;

        let acc = AccountService::new(db.clone());
        let acc_id = acc.create("test", "hunter3", Region::Europe, true).await?;
